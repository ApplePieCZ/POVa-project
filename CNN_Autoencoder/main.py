...
def knn_search(encoded_input, encoded_dataset, k=5):
    nbrs = NearestNeighbors(n_neighbors=k, metric='euclidean').fit(encoded_dataset)
    distances, indices = nbrs.kneighbors([encoded_input])
    return indices[0]


def find_similar_images(model, dataset_path, encoded_dataset_path, input_image_path, k=5):
    autoencoder_model = load_autoencoder_model('conv_autoencoder.pth')
    encoded_input = encode_image(autoencoder_model, input_image_path).flatten()
...
if TRAIN:
    ...
    input_image_path = r"flowers\flowers-102\jpg\image_00002.jpg"
    if not STORE:
        find_similar_images(autoencoder_model, dataset_path + r"\flowers-102\jpg", en_dataset_path, input_image_path)
        exit()
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
